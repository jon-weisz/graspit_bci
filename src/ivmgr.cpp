...
/*
  * Aligns the hand to face the object
  */
void
IVmgr::align() {
		  ...
		  transf objTran;
      if(world->getCurrentHand()->getGrasp()->getObject())
        objTran= world->getCurrentHand()->getGrasp()->getObject()->getTran();
      else
        objTran= world->getGB(0)->getTran();
		  transf robotTran = world->getCurrentHand()->getTran();
		  vec3 robToObj = objTran.translation() - robotTran.translation();

		  //normalize vector b/w rob and obj so we can use coords as trig functions
		  vec3 dirNorm = normalise(robToObj);
		  //do same but for XZ plane so we can rotate around Y
		  vec3 dirNormXZ = vec3(dirNorm.x(), 0, dirNorm.z());
		  vec3 dirNormXZNorm = normalise(dirNormXZ);


		  //make rotation matrices around Y, then X
		  double * rotationY = new double[9];
		  rotationY[0] = dirNormXZNorm.z();	 rotationY[3] = 0;	rotationY[6] = -dirNormXZNorm.x();
		  rotationY[1] = 0; 			     rotationY[4] = 1;	rotationY[7] = 0;
		  rotationY[2] = dirNormXZNorm.x();  rotationY[5] = 0; 	rotationY[8] = dirNormXZNorm.z();

		  double * rotationX = new double[9];
		  rotationX[0] = 1;			rotationX[3] = 0;					rotationX[6] = 0;
		  rotationX[1] = 0; 		rotationX[4] = dirNormXZ.len();		rotationX[7] = -dirNorm.y();
		  rotationX[2] = 0;  		rotationX[5] = dirNorm.y(); 		rotationX[8] = dirNormXZ.len();

		  mat3 rotX = mat3(rotationX);
		  mat3 rotY = mat3(rotationY);

		  //matrix for total rotation
		  mat3 rotationToObj = rotX*rotY;

		  //create new transforms and set
		  transf newTran = transf(rotationToObj, robotTran.translation());
...
/*
 * Rotates the hand around the object's Z-axis and maintains the approach
...
/*
 * Rotates the hand around the object's x-axis and maintains the approach
 * direction pointing at the object
 */
void
IVmgr::rotateLong() {
